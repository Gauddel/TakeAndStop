...
const ConditionBalance = require("../pre-compiles/ConditionBalance.json");
const ProviderModuleDSA = require("../pre-compiles/ProviderModuleDSA.json");
const IERC20 = require("../pre-compiles/IERC20.json");

describe("Stop Loss strategy for ETH/USD decreas with Condition Balance", function() {
    this.timeout(0);
    ...
    before(async function() {
        ...
        conditionBalance = await ethers.getContractAt(
            ConditionBalance.abi,
            bre.network.config.ConditionBalance
        )
    ...
    it("Stop Loss if Ether price is too low against USD", async function() {
        ...
        const checkBalanceCondition = new GelatoCoreLib.Condition({
        ...
        ).to.be.equal("ConditionNotOk:NotOkETHBalanceIsNotGreaterThanRefBalance");
